...
import { View, Text, Slider, StyleSheet, ImageBackground, Alert } from 'react-native';
...
export default class Configuracion extends React.Component {
    ...
    async componentWillUnmount() {
        let volInfo = await getVolume();
        if(volInfo.music !== this.state.music || volInfo.fx !== this.state.fx){
            Alert.alert('Nivel de volumen cambiado',
            'Para usar estos nuevos niveles, por favor reinicie el juego',
            [{
                text: 'OK', onPress: () => {
                    console.log('Arreglar esto pronto');
                }
            }]);
        }
